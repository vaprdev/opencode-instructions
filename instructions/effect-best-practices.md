...
## 1. Map the Effect Architecture First

- During planning, map the entrypoint, service calls, layer wiring, typed errors, resource lifecycles, and execution boundary affected by the change.
- Identify existing repository patterns before proposing a new service, layer, schema, or error type.
- Distinguish current-state and proposed-state call graphs when a change affects orchestration.
- Apply these guidelines during design and review, but do not begin implementation until the user explicitly requests changes.

## 2. Model Programs as Values
...
## 5. Build Dependencies with Layers
...
## 6. Validate Boundaries with Schema
